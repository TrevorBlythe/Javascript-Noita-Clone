...
let stepEntitiesPhysics = function(x,y,width,height){
  ...
  for(let i = 0;i<entities.length;i++){
    ...
    if(!collidingUp(entities[i]) && entities[i].yVel < 0){
      entities[i].y += entities[i].yVel
    }
    if(!collidingDown(entities[i]) && entities[i].yVel > 0){
      entities[i].y += entities[i].yVel
    }
    entities[i].xVel = lerp(entities[i].xVel,0,.2)
    entities[i].yVel = lerp(entities[i].yVel,0,.05)
...
let collidingDown = function(thing){
  let part = Math.floor(particles[Math.floor(thing.y + thing.height)][Math.floor(thing.x)]);
  let part2 = Math.floor(particles[Math.floor(thing.y + thing.height)][Math.floor(thing.x + thing.width)]);
...
let collidingUp = function(thing){
  let part = Math.floor(particles[Math.floor(thing.y)][Math.floor(thing.x)])
  let part2 = Math.floor(particles[Math.floor(thing.y)][Math.floor(thing.x + thing.width)])
  if(pProps[part].s == "s" || pProps[part2].s == "s"){
    return true;
  }else{
    return false;
...
let collidingRight = function(thing){
  let part = Math.floor(particles[Math.floor(thing.y + 1)][Math.floor(thing.x + thing.width + 1)])
  let part2 = Math.floor(particles[Math.floor(thing.y + thing.height - 1)][Math.floor(thing.x + thing.width + 1)])
  if(pProps[part].s == "s" || pProps[part2].s == "s"){
    return true;
  }else{
    return false;
...
let collidingLeft = function(thing){
  let part = Math.floor(particles[Math.floor(thing.y+1)][Math.floor(thing.x-1)])
  let part2 = Math.floor(particles[Math.floor(thing.y + thing.height-1)][Math.floor(thing.x-1)])
  if(pProps[part].s == "s" || pProps[part2].s == "s"){
    return true;
  }else{
    return false;
