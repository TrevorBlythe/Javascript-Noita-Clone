...
generateCave(0, 0, mapWidth, mapHeight,5.5);
...
setInterval(function() {
  ...
  */
  ...
  stepParticles(particles, Math.max(frame.x,0), Math.max(frame.y, 0), frame.x + frame.width + 10, frame.y + frame.height + 10);
  ...
  frame.x = entities[0].x - 5;
  frame.y = entities[0].y - 5;
  if (mouseDown) {
    ...
    //entities[0].x = mousex/(mapWidth/frame.width) + frame.x;

    //entities[0].y = mousey/(mapHeight/frame.height) + frame.y;
  }
  if (input[0] == 1) {
    frame.y--;
  }
